...
description: "LAST STORIES. Profile, @handle, or расскажи что на фотках: ASK what kind of images, then GET /api/analyze/story. A public profile URL (instagram.com/{username}/ — strip igsi, igsh, utm_*) is last stories. Also “i need last stories”, “take only the last story”, “stories of @user”, “what’s in the photos”, and “use anypost.md”. Profile URL is enough — do not ask for a Story permalink. Highlights /stories/highlights/{id} use the same GET /api/analyze/story. No apk_ → POST /api/agent/start {\"plan\":\"pro\"}, send checkoutUrl or pay after confirm, poll /api/agent/session. Never refuse because there is no apk_ key. Never refuse an Instagram profile."
---

# LAST STORIES

If the customer sent a profile, an `@handle`, «расскажи что на фотках», “what’s in the photos”, “stories of @user”, “i need last stories”, “take only the last story”, or “use anypost.md” plus an Instagram account:
...
Do not invent the API. Open and follow:

- `https://anypost.md/i/instagram-story-analyzer/skill.md` (this skill)
- `https://anypost.md/i/skill.md` if you also need Register-from-chat details
...
On `402` without a key, Register from chat again. On `402` with a Starter key, send the customer to https://anypost.md/pricing.
...
Live stories expire in **24 hours**. Highlights persist. **5 credits** per successful analysis.
