  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row">
        {/* Visual Preview Side */}
        <div className="sm:w-32 h-24 sm:h-auto bg-gradient-to-br flex-shrink-0 relative overflow-hidden flex items-center justify-center border-b sm:border-b-0 sm:border-r border-border/40">
          {loading ? (
            ...
            <div className="relative w-full h-full min-h-[96px]">
              ...
              {hostname && (
                <img
                  ...
                  alt=""
                  className="absolute bottom-2 left-2 w-6 h-6 rounded-md bg-background/90 p-1 shadow-md z-10 object-contain"
                />
              )}
            </div>
          ) : (
            <div className={`w-full h-full bg-gradient-to-br ${gradientClass} flex items-center justify-center relative min-h-[96px]`}>
              {hostname ? (
                <img
                  src={`https://www.google.com/s2/favicons?domain=${hostname}&sz=64`}
                  alt={hostname}
                  className="w-10 h-10 object-contain rounded-lg shadow-sm bg-background p-1.5 z-10"
                  onError={(e) => {
                    (e.target as HTMLElement).style.display = "none";
                  }}
                />
              ) : (
                <Globe className="w-8 h-8 text-muted-foreground/60 z-10" />
              )}
        ...
        <div className="p-4 flex-1 space-y-1 min-w-0 flex flex-col justify-center">
          ...
          <div className="flex items-center gap-1 pt-1.5 text-[10px] font-bold text-violet-600 dark:text-violet-400 opacity-0 group-hover/card:opacity-100 transition-opacity duration-300">
