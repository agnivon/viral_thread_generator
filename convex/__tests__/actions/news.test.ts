...
test("googleTrends - getTrendingKeywords fetches live trends via @alkalisummer/google-trends-js on-demand", async () => {
  ...
  const result = await authedT.action(api.actions.googleTrends.getTrendingKeywords, {
    geo: "US",
    hours: 24,
  });
...
test("googleTrends - fetchArticlesForKeyword fetches articles on-demand using articleKeys", async () => {
  const t = convexTest(schema, modules);
  const userId = await t.mutation(async (ctx) => ctx.db.insert("users", {}));
  const authedT = t.withIdentity({ subject: userId });
  ...
  const result = await authedT.action(api.actions.googleTrends.fetchArticlesForKeyword, {
    keyword: "rams",
    articleKeys: [[4800202104, "en", "US"]],
  });
...
test("googleTrends - fetchArticlesForKeyword resolves articleKeys via realTimeTrends if omitted", async () => {
  const t = convexTest(schema, modules);
  const userId = await t.mutation(async (ctx) => ctx.db.insert("users", {}));
  const authedT = t.withIdentity({ subject: userId });
