...
test("threads token refresh flow - refreshes when near expiry", async () => {
  ...
  const initialToken = await t.query(internal.tokens.getLatestToken, {
    platform: "threads",
    type: "long lived",
    userId,
  });
  ...
  const initialTokenId = await t.mutation(internal.tokens.updateToken, {
  ...
  const activeToken = await t.query(internal.tokens.getLatestToken, {
    platform: "threads",
    type: "long lived",
    userId,
  });
  ...
  const result = await t.action(internal.actions.tokens.refreshThreadsToken, { userId });
...
test("storeAuthToken and deleteTokensByPlatform mutations", async () => {
  ...
  await t.mutation(internal.tokens.updateToken, {
    userId, platformUserId: "platform-12345",
  ...
  await t.mutation(internal.tokens.deleteTokensByPlatform, {
    platform: "threads",
    userId,
  });
  ...
  const shortLivedId = await t.mutation(internal.tokens.storeAuthToken, {
    userId, platformUserId: "platform-12345",
    platform: "threads",
  ...
  const longLivedId = await t.mutation(internal.tokens.storeAuthToken, {
    userId, platformUserId: "platform-12345",
    platform: "threads",
...
test("hasActiveToken public query", async () => {
  ...
  const unauthHasToken = await t.query(api.tokens.hasActiveToken, {
    platform: "threads",
    type: "long lived",
  });
  ...
  const hasTokenAfter = await tUser.query(api.tokens.hasActiveToken, {
    platform: "threads",
    type: "long lived",
  });
...
test("getTokensNearExpiry query returns tokens near expiry and null otherwise", async () => {
  ...
  const initialCheck = await t.query(internal.tokens.getTokensNearExpiry, {
    userId,
    platform: "threads",
    type: "long lived",
    now,
  ...
  const tokenIdNear = await t.mutation(internal.tokens.updateToken, {
    userId,
    platformUserId: "p1",
    newToken: "token-near",
  ...
  const checkNear = await t.query(internal.tokens.getTokensNearExpiry, {
    userId,
    platform: "threads",
    type: "long lived",
    now,
  ...
  const checkInactive = await t.query(internal.tokens.getTokensNearExpiry, {
    userId,
    platform: "threads",
    type: "long lived",
    now,
  ...
  const checkFar = await t.query(internal.tokens.getTokensNearExpiry, {
    userId,
    platform: "threads",
    type: "long lived",
    now,
...
test("getAllTokensNearExpiry query returns all active tokens near expiry", async () => {
  ...
  const checkNear = await t.query(internal.tokens.getAllTokensNearExpiry, {
    platform: "threads",
    type: "long lived",
    now,
    nearExpiryLimit,
...
test("refreshAllThreadsTokens action bulk refreshes tokens near expiry", async () => {
  ...
  const tokenIdFar = await t.mutation(internal.tokens.updateToken, {
    userId: userId2,
    platformUserId: "p2",
    newToken: "token-far",
    expiresIn: 10 * 24 * 3600, // 10 days
  ...
  const result = await t.action(internal.actions.tokens.refreshAllThreadsTokens, {});
  expect(result.refreshedCount).toBe(1);
  expect(result.errors.length).toBe(0);

  // 5. Verify database state
  const user1Token = await t.query(internal.tokens.getLatestToken, {
    platform: "threads",
    type: "long lived",
    userId: userId1,
  });
  ...
  const user2Token = await t.query(internal.tokens.getLatestToken, {
    platform: "threads",
    type: "long lived",
    userId: userId2,
  });
