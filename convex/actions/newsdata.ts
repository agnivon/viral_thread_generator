...
import { db } from "../lib/clients/firebase.js";
import { NewsDataAPI } from "../lib/clients/newsdata.js";
...
export const deleteOldNewsArticles = internalAction({
  ...
  handler: async () => {
    ...
    console.log(
      `Successfully deleted a total of ${totalDeleted} old news articles across ${collectionsWithDeletions} keywords (${topLevelDocs.length} keywords checked).`
    );
  },
});
