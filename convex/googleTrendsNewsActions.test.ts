...
import { isValidArticleKey } from "./actions/googleTrendsNewsActions.js";
...
test("getTrendingKeywords - returns formatted trends for authenticated user", async () => {
  ...
  const result = await authed.action(api.actions.googleTrendsNewsActions.getTrendingKeywords, {
    geo: "US",
  });
...
test("getTrendingKeywords - throws Unauthorized when not logged in", async () => {
  ...
  await expect(
    t.action(api.actions.googleTrendsNewsActions.getTrendingKeywords, {})
...
test("fetchArticlesForKeyword - returns formatted articles for authenticated user", async () => {
  ...
  const authed = t.withIdentity({ subject: userId });
  const articles = await authed.action(api.actions.googleTrendsNewsActions.fetchArticlesForKeyword, {
    keyword: "Artificial Intelligence",
