...
import {
  ...
  deepSeekFlashT085ReasoningNone, deepSeekFlashT00ReasoningHigh,
...
const socialMediaThreadWriterModels = [
  ...
  deepSeekFlashT085ReasoningNone.withStructuredOutput(socialMediaWriterSchema, { name: "thread_writer", method: "jsonMode" }),
  ...
  try {
    draft = await invokeWithFallbacks(
      ...
      [
        ...
        { role: "user", content: `<HOOK>\n${state.selected_hook}\n</HOOK>\n\n<SOURCE>\n${state.raw_markdown}\n</SOURCE>${researchContextStr}${deltaContext}${previousDraftContext}${critiqueContext}${postCritiquesContext}${charCritiqueContext}${guidanceContext}` }
      ],
