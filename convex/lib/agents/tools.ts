...
export interface ThreadValidationOptions {
  thread_draft: string[];
  check_line_breaks?: boolean;
  ignoreSoftLimits?: boolean;
}

export function validateThreadDraftContent(options: ThreadValidationOptions): { isValid: boolean; errors: string[] } {
  const { thread_draft, check_line_breaks, ignoreSoftLimits } = options;
  const errors: string[] = [];
  let over280Count = 0;
  ...
  if (!ignoreSoftLimits && over280Count > 3) {
...
export const CharacterValidatorTool = tool(
  async ({ thread_draft, check_line_breaks }) => {
    const result = validateThreadDraftContent({ thread_draft, check_line_breaks });
